...
namespace DemoBrowser
{
    public partial class MainForm : Form
    {
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (MessageUrlList.Count >= 1)
                {
                    defaultUrl = MainBrowser.WebView.Url = MessageUrlList[0].Item2;
                }

                if (MessageUrlList.Count >= 2)
                {
                    MainBrowser2.WebView.Url = MessageUrlList[1].Item2;
                }
            }
            catch(Exception ex)
            {
        ...
        private void ThreadCallBack()
        {
            while (isThreadRunning)
            {
                ...
                if(MessageQueue.TryTake(out message,1000))
                {
                    ...
                    for (int i = 0; i < MessageUrlList.Count; i++)
                    {
                        if (StringUtils.EqualsEx(message, MessageUrlList[i].Item1))
                        {
                            index = i;
                        }
                    }
                        ...
                        if (!MainBrowser.Disposing && !MainBrowser.IsDisposed)
                        {
                            MainBrowser.Invoke(new Action(() =>
                            {
                                try
                                {
                                    ...
                                    {
                                        MainBrowser2.Visible = true;
                                        MainBrowser.Visible = false;
                                    }
                                }
                                catch (Exception ex)
                                {
                                    NLogHelper.Error(string.Format("导航到页面{0}失败：{1}",messageUrlPair.Item2,ex));
                                }
                            }));
                        }
                    }
        ...
        private void webView1_KeyUp(object sender, EO.WebBrowser.WndMsgEventArgs e)
        {
            if (e.WParam.ToInt32() == 13)
            {
                FullScreen();
            }
            else if (e.WParam.ToInt32() == 27)
            {
                NormalScreen();
            }
        }
        ...
        private void webView2_KeyUp(object sender, EO.WebBrowser.WndMsgEventArgs e)
        {
            if (e.WParam.ToInt32() == 13)
            {
                FullScreen();
